...
def initDT(dtDict):
    ...
    assert (
        TCP_PORT == 502 or 5000 < TCP_PORT < 10000
    ), "port should either be 502 or in (5000,10000)"

    if "update" in dtDict:
        UPDATE_RATE = dtDict["update"]

    assert (
        0.0 < UPDATE_RATE < 10.0
    ), "update should be positive and less than 10 seconds"
    ...
    assert (
        0 <= TANK_LEVEL <= MAX_TANK_LEVEL
    ), "initial level should be in [0,{}]".format(MAX_TANK_LEVEL)

    if updates == 0 and "highLevel" in dtDict:
        TANK_HIGH_LEVEL = dtDict["highLevel"]

    assert (
        0 < TANK_HIGH_LEVEL <= MAX_TANK_LEVEL
    ), "highLevel should be in (0,{}]".format(MAX_TANK_LEVEL)

    if updates == 0 and "lowLevel" in dtDict:
        TANK_LOW_LEVEL = dtDict["lowLevel"]

    assert 0 < TANK_LOW_LEVEL < TANK_HIGH_LEVEL, "highLevel should be in (0,{}]".format(
        TANK_HIGH_LEVEL
    )
...
def update_tank_state(context):
    ...
    if (
        TANK_STATE["coils"][COIL_MAP["INPUT"]] > 0
        and TANK_STATE["inputs"][INPUT_MAP["BRB"]] > 0
    ):
    ...
    if (
        TANK_STATE["coils"][COIL_MAP["OUTPUT"]] > 0
        and TANK_STATE["inputs"][INPUT_MAP["DRAIN"]] > 0
    ):
...
async def updating_task(context):
    ...
    context[SLAVE_ID].getValues(
        rd_reg_as_hex, rd_reg_address, count=len(TANK_STATE["registers"])
    )
    context[SLAVE_ID].setValues(rd_reg_as_hex, rd_reg_address, TANK_STATE["registers"])

    context[SLAVE_ID].getValues(
        rd_output_coil_as_hex, rd_output_coil_address, count=len(TANK_STATE["coils"])
    )
    context[SLAVE_ID].setValues(
        rd_output_coil_as_hex, rd_output_coil_address, TANK_STATE["coils"]
    )

    context[SLAVE_ID].getValues(
        rd_direct_input_as_hex, rd_direct_input_address, count=len(TANK_STATE["inputs"])
    )
    context[SLAVE_ID].setValues(
        rd_direct_input_as_hex, rd_direct_input_address, TANK_STATE["inputs"]
    )
    ...
    while True:
        ...
        coil_values = context[SLAVE_ID].getValues(
            rd_output_coil_as_hex,
            rd_output_coil_address,
            count=len(TANK_STATE["coils"]),
        )
        print("coil values", coil_values)

        input_values = context[SLAVE_ID].getValues(
            rd_direct_input_as_hex,
            rd_direct_input_address,
            count=len(TANK_STATE["inputs"]),
        )
        ...
        context[SLAVE_ID].setValues(
            rd_reg_as_hex, rd_reg_address, TANK_STATE["registers"]
        )
...
if __name__ == "__main__":
    ...
    if not len(sys.argv) < 2:
        ...
        ARG_FILE = os.path.join(".", sys.argv[1])
        try:
            with open(ARG_FILE, "r") as rf:
                dtDict = json.load(rf)
        except:
